...
export class ClassicCtfMode implements GameMode {
  ...
  initialize(world: WorldState): readonly GameEvent[] {
    ...
    world.objectives = [
      createTeamFlag("red", this.map.presentation.redBase),
      createTeamFlag("blue", this.map.presentation.blueBase),
    ];
...
function createTeamFlag(
  teamId: TeamId,
  base: WorldMapPresentationRect,
): Objective {
  return {
    id: `${teamId}-flag`,
    kind: "team-flag",
    position: centerOf(base),
    state: {
      status: "home",
